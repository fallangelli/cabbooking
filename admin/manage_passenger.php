...
if (isset($_REQUEST['arr_ids'])) {
    ...
    if (is_array($arr_ids)) {
        ...
        if (isset($_REQUEST['Delete'])) {
            ...
            db_query($sql);
            set_session_msg("Selected passenger have been deleted successfully");
        } else if (isset($_REQUEST['Activate'])) {
            $sql = "update tbl_user  set status = 'Active' where id in ($str_ids)";
            db_query($sql);
            set_session_msg("Selected passenger have been activated successfully");
        } else if (isset($_REQUEST['Deactivate'])) {
            $sql = "update tbl_user set status = 'Inactive' where id in ($str_ids)";
            db_query($sql);
            set_session_msg("Selected passenger have been deactivated successfully");
        }
    }
...
<?php include("top.inc.php"); ?>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            <td id="pageHead">
                <div id="txtPageHead">Manage Passenger</div>
            </td>
        </tr>
    </table>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            <td id="content" align="left">
                ...
                <p align="center"></p>
                <div align="right"><a href="passenger_add.php?set_flag=add&start=<?= $start ?>">Add New Passenger </a>
                </div>
                <p></p>
                            ...
                            while ($line_raw = ms_stripslashes(mysqli_fetch_array($result))) {
                                ...
                                <tr class="<?= $css ?>">
                                    ...
                                    <td align="center" valign="top"><a
                                                href="passenger_add.php?id=<?= $line_raw['id'] ?>&set_flag=update<?= $_REQUEST[parent_id] != '' ? '&parent_id=' . $_REQUEST[parent_id] : '' ?>"><img
                                                    src="images/icons/edit.png" alt="Edit" width="16" height="16"
                                                    border="0"/></a></td>
                                    <td align="left" valign="top"><?= stripslashes($line_raw['fullname']); ?><br/>
                                        <a href='javascript:;'
                                           onClick="window.open('view_passenger.php?pid=<?= $line_raw['id']; ?>', '_blank', 'toolbar=no,menubar=no,scrollbars=yes,resizable=1,height=500,width=750');">View
                                            All</a></td>
