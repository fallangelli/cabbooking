...
if (isset($_REQUEST["unique_id"]) && isset($_REQUEST["user_type"]) && isset($_REQUEST["confirm_reject_cab"])) {
    ...
    $sel_stat = mysqli_fetch_object(db_query("select * from tbl_ride where id='$unique_id'"));
    $stat = $sel_stat->ride_status;
    $driver_id = $sel_stat->driver;
    $passenger_id = $sel_stat->passenger;

    $sel_driver_data = mysqli_fetch_object(db_query("select * from tbl_user where id='$driver_id'"));
    $driver_email = $sel_driver_data->email;

    $sel_pass_data = mysqli_fetch_object(db_query("select * from tbl_user where id='$passenger_id'"));
    ...
    if ($stat != 'reject') {
        ...
        $update_status_exe = db_query($update_status);

        if ($stat == 'confirm') {
            if ($ride_status == 'reject') {
                if ($user_type == 'driver') {
                    $sel_query = db_query("SELECT * FROM gcm_users where email='$user_email' ");
    ...
    $sel_values = db_query("select * from tbl_ride where id='$unique_id' and ride_status='confirm' ");
    ...
    if ($sel_count > 0) {
        ...
        $sel_pay_values = db_query("select * from tbl_payments where passenger_id='$passenger_id' and driver_id='$driver_id' and pickup_date='$pickup_date' and pickup_time='$pickuptime'");
